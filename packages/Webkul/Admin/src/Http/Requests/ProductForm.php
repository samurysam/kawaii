...
class ProductForm extends FormRequest
{
    ...
    public function __construct(
        ...
        $configuredLimit = core()->getConfigData('catalog.products.attribute.file_attribute_upload_size');

        $this->maxVideoFileSize = $configuredLimit
            ? (int) $configuredLimit
            : $this->resolvePhpUploadLimitKilobytes();
    }
    ...
    /**
     * Fall back to PHP's effective upload ceiling when Bagisto has no explicit limit set.
     */
    protected function resolvePhpUploadLimitKilobytes(): int
    {
        ...
        return $limits ? min($limits) : 2048;
    }

    /**
     * Convert shorthand INI values like 10M, 512K, or 1G into kilobytes.
     */
    protected function iniSizeToKilobytes(string $value): int
    {
        ...
        if (! preg_match('/^(?<size>\d+(?:\.\d+)?)(?<unit>[KMG]?)$/i', $value, $matches)) {
            return 0;
        }
        ...
        return match ($unit) {
            'G' => (int) round($size * 1024 * 1024),
            'M' => (int) round($size * 1024),
            'K' => (int) round($size),
            default => (int) round($size / 1024),
        };
    }
}

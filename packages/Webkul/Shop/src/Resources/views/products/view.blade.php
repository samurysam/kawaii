...
@php
    ...
    $productVideos = product_video()->getVideos($product);

    $featuredProductVideo = $productVideos[0] ?? null;
@endphp
...
<x-shop::layouts>
    ...
    <!-- Breadcrumbs -->
    <div class="kb-product-page">
    @if ((core()->getConfigData('general.general.breadcrumbs.shop')))
        <div class="kb-product-breadcrumbs flex justify-center px-7 max-lg:hidden">
            <x-shop::breadcrumbs
                name="product"
                :entity="$product"
            />
    ...
    <div class="1180:mt-20">
        <div class="container max-1180:hidden px-[60px] max-lg:px-8">
            <x-shop::tabs
                class="kb-product-info-tabs"
                position="center"
                ref="productTabs"
            >
                <!-- Description Tab -->
                ...
                >
                    <div class="container mt-[60px] max-1180:px-5">
                        <p class="kb-product-info-copy text-lg max-1180:text-sm">
                            {!! $product->description !!}
                        </p>
                    </div>
                </x-shop::tabs.item>
                            ...
                            <div class="mt-8 grid max-w-max grid-cols-[auto_1fr] gap-4">
                                @foreach ($customAttributeValues as $customAttributeValue)
                                    @if (! empty($customAttributeValue['value']))
                                        ...
                                        @else
                                            <div class="grid">
                                                <p class="kb-product-info-copy text-base">
    ...
    <div class="container mt-6 grid gap-3 !p-0 max-1180:px-5 1180:hidden">
        ...
        <x-shop::accordion
            class="kb-product-info-tabs max-md:border-none"
            :is-active="true"
        >
            <x-slot:header class="bg-[linear-gradient(135deg,_#fff6fa_0%,_#fad9e5_100%)] max-md:!py-3 max-sm:!py-2">
            ...
            <x-slot:content class="max-sm:px-0">
                <div class="kb-product-info-copy mb-5 text-lg max-1180:text-sm max-md:mb-1 max-md:px-4">
                    {!! $product->description !!}
                </div>
            </x-slot>
        </x-shop::accordion>
            ...
            >
                ...
                <x-slot:content class="max-sm:px-0">
                    <div class="container max-1180:px-5">
                        <div class="grid max-w-max grid-cols-[auto_1fr] gap-4 text-lg text-[#8A6772] max-1180:text-sm">
                            @foreach ($customAttributeValues as $customAttributeValue)
                                @if (! empty($customAttributeValue['value']))
                                    <div class="grid">
                                        <p
                                            class="text-base text-[#5B3A45]"
                                            v-pre
                                        >
                                            {{ $customAttributeValue['label'] }}
                                        </p>
        ...
        >
            <x-slot:header
                class="bg-[linear-gradient(135deg,_#fff6fa_0%,_#fad9e5_100%)] max-md:!py-3 max-sm:!py-2"
            ...
            >
                ...
                >
                    ...
                    <div class="container px-[60px] max-1180:px-0">
                        <div class="kb-product-stage mt-12 flex gap-9 max-1180:flex-wrap max-lg:mt-0 max-sm:gap-y-4">
                            <!-- Gallery Blade Inclusion -->
                            @include('shop::products.view.gallery')

                            <!-- Details -->
                            <div class="kb-product-details relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                {!! view_render_event('bagisto.shop.products.name.before', ['product' => $product]) !!}

                                <div class="flex justify-between gap-4">
                                    <h1 class="kb-product-title break-words max-sm:text-xl" v-pre>
                                        {{ $product->name }}
                                    </h1>

                                    @if (core()->getConfigData('customer.settings.wishlist.wishlist_option'))
                                        <div
                                            class="kb-product-chip max-sm:min-h-7 max-sm:min-w-7 flex max-h-[46px] min-h-[46px] min-w-[46px] cursor-pointer items-center justify-center text-2xl transition-all hover:opacity-[0.8] max-sm:max-h-7 max-sm:text-base"
                                            role="button"
                                ...
                                @if ($totalRatings = $reviewHelper->getTotalFeedback($product))
                                    ...
                                    <div
                                        class="mt-3 w-max cursor-pointer max-sm:mt-1.5"
                                        role="button"
                                        tabindex="0"
                                ...
                                <p class="kb-product-price mt-[22px] flex items-center gap-2.5 text-2xl !font-medium max-sm:mt-2 max-sm:gap-x-2.5 max-sm:gap-y-0 max-sm:text-lg">
                                    {!! $product->getTypeInstance()->getPriceHtml() !!}
                                </p>

                                @if (\Webkul\Tax\Facades\Tax::isInclusiveTaxProductPrices())
                                    <span class="text-sm font-normal text-[#8A6772] max-sm:text-xs">
                                ...
                                @if (count($product->getTypeInstance()->getCustomerGroupPricingOffers()))
                                    <div class="mt-2.5 grid gap-1.5">
                                        @foreach ($product->getTypeInstance()->getCustomerGroupPricingOffers() as $offer)
                                            <p class="text-[#8A6772] [&>*]:text-[#5B3A45]">
                                                {!! $offer !!}
                                            </p>
                                        @endforeach
                                    </div>
                                ...
                                <div class="kb-product-short-description mt-6 max-sm:mt-1.5 max-sm:text-sm">
                                    {!! $product->short_description !!}
                                </div>

                                {!! view_render_event('bagisto.shop.products.short_description.after', ['product' => $product]) !!}

                                @if ($featuredProductVideo)
                                    <div class="kb-product-video-edge">
                                        <div class="kb-product-video-frame">
                                            <video
                                                class="w-full"
                                                autoplay
                                                muted
                                            ...
                                            >
                                                <source
                                                    src="{{ $featuredProductVideo['video_url'] }}"
                                                    type="video/mp4"
                                                />
                                            </video>
                                        </div>
                                ...
                                <div class="mt-10 flex gap-9 max-md:mt-4 max-md:flex-wrap max-sm:justify-center max-sm:gap-3">
                                    ...
                                    <div
                                        class="kb-product-chip flex cursor-pointer items-center justify-center gap-2.5 px-4 py-3 max-sm:gap-1.5 max-sm:text-base"
                                        role="button"
                                        tabindex="0"

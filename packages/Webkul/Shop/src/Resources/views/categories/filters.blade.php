...
<div
    class="fixed bottom-0 z-10 grid w-full max-w-full grid-cols-[1fr_auto_1fr] items-center justify-items-center border-t border-[#F4C7D6] bg-white/90 px-5 backdrop-blur-md ltr:left-0 rtl:right-0"
    v-if="isMobile"
>
    <!-- Filter Drawer -->
    <x-shop::drawer
    ...
    >
        ...
        <x-slot:toggle>
            <div
                class="flex cursor-pointer items-center gap-x-2.5 px-2.5 py-3.5 text-base font-medium uppercase text-[#5B3A45] max-md:py-3"
        ...
        <x-slot:header>
            <div class="flex items-center justify-between">
                ...
                <p
                    class="cursor-pointer text-sm font-medium text-[#ED6E98] ltr:mr-[50px] rtl:ml-[50px]"
    ...
    <!-- Separator -->
    <span class="h-5 w-0.5 bg-[#F4C7D6]"></span>
...
@pushOnce('scripts')
    ...
    >
        ...
        <!-- Filters Container -->
        <template v-else>
            <div class="kb-filter-panel panel-side journal-scroll grid max-h-[1320px] min-w-[342px] grid-cols-[1fr] overflow-y-auto overflow-x-hidden p-6 max-xl:min-w-[270px] md:max-w-[342px] md:ltr:pr-6 md:rtl:pl-6">
                <!-- Filters Header Container -->
                <div class="flex h-[50px] items-center justify-between border-b border-[#F6D8E4] pb-2.5 max-md:hidden">
                    <p class="text-lg font-semibold text-[#5B3A45] max-sm:font-medium">
                        @lang('shop::app.categories.filters.filters')
                    </p>

                    <p
                        class="cursor-pointer text-xs font-medium text-[#ED6E98]"
                        tabindex="0"
                        @click="clear()"
                    >
                        @lang('shop::app.categories.filters.clear-all')
    ...
    >
        <x-shop::accordion class="last:border-b-0 border-b border-[#F8DDE8] py-1">
            <!-- Filter Item Header -->
            <x-slot:header class="px-0 py-2.5 max-sm:!pb-1.5">
                <div class="flex items-center justify-between">
                    <p class="text-lg font-semibold text-[#5B3A45] max-sm:text-base max-sm:font-medium">
                        @{{ filter.name }}
                    </p>
                </div>
            </x-slot>
            ...
            <x-slot:content class="!p-0">
                ...
                <template v-else>
                    ...
                    >
                        <div class="relative">
                            <div class="icon-search pointer-events-none absolute top-3 flex items-center text-2xl text-[#B08D98] max-md:text-xl max-sm:top-2.5 ltr:left-3 rtl:right-3"></div>

                            <input
                                type="text"
                                class="kb-filter-input block w-full px-11 py-3.5 text-sm font-medium max-md:rounded-lg max-md:px-10 max-md:py-3 max-md:font-normal max-sm:text-xs"
                        ...
                        <p
                            class="mt-1 flex flex-row-reverse text-xs text-[#8A6772]"
                            v-text="
                    ...
                    <!-- Filter Options -->
                    <ul class="pb-3 text-base text-[#5B3A45]">
                        <template v-if="options.length">
                            <li
                                :key="`${filter.id}_${option.id}`"
                                v-for="(option, optionIndex) in options"
                            >
                                <div class="kb-filter-option flex select-none items-center gap-x-4 rounded-[18px] max-sm:gap-x-1 max-sm:!p-0 ltr:pl-2 rtl:pr-2">
                                    <input
                                        type="checkbox"
                                    ...
                                    <label
                                        class="w-full cursor-pointer p-2 text-base text-[#5B3A45] max-sm:p-1 max-sm:text-sm ltr:pl-0 rtl:pr-0"
